...
export function createPhase3Ramp(ac: AudioContext, durationSec: number): GainNode {
  ...
  // Front-loaded ramp with heavy overdrive: reach 3.0 in first quarter, then 5.0.
  // Values above 1.0 push the output louder than unity gain — the browser
  // soft-clips which adds urgency. This ensures the wake alarm is much louder
  // than the gentle singing bowl (which plays at gain 1.0).
  const quarterSec = durationSec * 0.25;
  masterGain.gain.setValueAtTime(0, ac.currentTime);
  masterGain.gain.linearRampToValueAtTime(3.0, ac.currentTime + quarterSec);
  masterGain.gain.linearRampToValueAtTime(5.0, ac.currentTime + durationSec);
